...
def add_poi_ratio(data_dict, features_list):
    ...
    for item in data_dict:
        individual = data_dict[item]
        is_valid = True
        for feature in features:
            if individual[feature] == 'NaN':
                is_valid = False
        if is_valid:
            total_messages = individual['to_messages'] +\
                             individual['from_messages']
            poi_messages = individual['from_poi_to_this_person'] +\
                           individual['from_this_person_to_poi']
            individual['poi_ratio'] = float(poi_messages) / total_messages
        else:
            individual['poi_ratio'] = 'NaN'
    features_list += ['poi_ratio']


# calculates fraction of email communication tp POI                  
    ...
    for item in data_dict:
        ...
        for feature in features:
            if individual[feature] == 'NaN':
                is_valid = False
        if is_valid:
            total_messages = individual['from_messages']
            poi_messages =   individual['from_this_person_to_poi']
            individual['fraction_to_poi'] = float(poi_messages) / total_messages
        else:
            individual['fraction_to_poi'] = 'NaN'
    features_list += ['fraction_to_poi']

# calculates fraction of email communication from POI                  
def add_fraction_from_poi(data_dict, features_list):
    ...
    for item in data_dict:
        ...
        if is_valid:
            total_messages = individual['to_messages']
            poi_messages =   individual['from_poi_to_this_person']
            individual['fraction_from_poi'] = float(poi_messages) / total_messages
        else:
            individual['fraction_from_poi'] = 'NaN'
...
## add communication ratios
add_poi_ratio(my_dataset, features_list)
add_fraction_to_poi(my_dataset, features_list)
add_fraction_from_poi(my_dataset, features_list)
